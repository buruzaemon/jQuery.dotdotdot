...
{var e=$e[0];if(!e)
{return false;}
var tiny=new TinySegmenter();var txt=getTextContent(e),space=(txt.indexOf(' ')!==-1)?' ':'\u3000',separator=(o.wrap=='letter')?'':space,textArr=tiny.segment(txt),position=-1,midPos=-1,startPos=0,endPos=textArr.length-1;if(o.fallbackToLetter&&startPos==0&&endPos==0)
{separator='';textArr=tiny.segment(txt);endPos=textArr.length-1;}
while(startPos<=endPos&&!(startPos==0&&endPos==0))
{var m=Math.floor((startPos+endPos)/2);if(m==midPos)
{break;}
midPos=m;var chunk=joinChars(textArr.slice(0,midPos+1));setTextContent(e,chunk+o.ellipsis);$i.children().each(function()
{$(this).toggle().toggle();});if(!test($i,o))
{position=midPos;startPos=midPos;}
else
{endPos=midPos;if(o.fallbackToLetter&&startPos==0&&endPos==0)
{separator='';textArr=tiny.segment(textArr[0]);position=-1;midPos=-1;startPos=0;endPos=textArr.length-1;}}}
if(position!=-1&&!(textArr.length==1&&textArr[0].length==0))
{var chunk=joinChars(textArr.slice(0,position+1));txt=addEllipsis(chunk,o);setTextContent(e,txt);}
else
...
{idx=idx||0;var code=str.charCodeAt(idx);var hi,low;if(0xD800<=code&&code<=0xDBFF){hi=code
...
{var tmp=[];for(var i=0,l=tArr.length;i<l;i++){tmp.push(tArr[i]);if(i<l-1){var c1=fixedCharCodeAt(tArr[i],tArr[i].length-1);var c2=fixedCharCodeAt(tArr[i+1],0);if(isJaCharCode(c1)===false&&isJaCharCode(c2)==false){tmp.push(' ');}}}
return tmp.join('');}
...
var unicode_spaces=['\u0020','\u00A0','\u1680','\u180E','\u2000','\u2001','\u2002','\u2003','\u2004','\u2005','\u2006','\u2007','\u2008','\u2009','\u200A','\u200B','\u202F','\u205F','\u3000','\uFFFF'];var result=[];var seg=["B3","B2","B1"];var ctype=["O","O","O"];var whsp=[' ','\u3000'];var o=input.split("");for(i=0;i<o.length;++i){seg.push(o[i]);ctype.push(this.ctype_(o[i]));}
