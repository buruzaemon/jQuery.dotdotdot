...
else{watchOrg=getSizes($dot);watchInt=setInterval(function(){if($dot.is(':visible')){var watchNew=getSizes($dot);if(watchOrg.width!==watchNew.width||watchOrg.height!==watchNew.height)
...
var tiny=new TinySegmenter();var txt=getTextContent(e),space=(txt.indexOf(' ')!==-1)?' ':'\u3000',separator=(o.wrap=='letter')?'':space,textArr=tiny.segment(txt),position=-1,midPos=-1,startPos=0,endPos=textArr.length-1;if(o.fallbackToLetter&&startPos===0&&endPos===0){separator='';textArr=tiny.segment(txt);endPos=textArr.length-1;}
while(startPos<=endPos&&!(startPos===0&&endPos===0)){var m=Math.floor((startPos+endPos)/2);if(m===midPos){break;}
midPos=m;var chunk=joinChars(textArr.slice(0,midPos+1));setTextContent(e,chunk+o.ellipsis);$i.children().each(function(){$(this).toggle().toggle();});if(!test($i,o)){position=midPos;startPos=midPos;}
else{endPos=midPos;if(o.fallbackToLetter&&startPos===0&&endPos===0){separator='';textArr=tiny.segment(textArr[0]);position=-1;midPos=-1;startPos=0;endPos=textArr.length-1;}}}
if(position!==-1&&!(textArr.length===1&&textArr[0].length===0)){var chunk=joinChars(textArr.slice(0,position+1));txt=addEllipsis(chunk,o);setTextContent(e,txt);}
...
h-=m;}
return h;}
function fixedCharCodeAt(str,idx){idx=idx||0;var code=str.charCodeAt(idx);var hi,low;if(0xD800<=code&&code<=0xDBFF){hi=code
low=str.charCodeAt(idx+1);if(isNaN(low)){throw'High surrogate not followed by low surrogate in fixedCharCodeAt()';}
return((hi-0xD800)*0x400)+(low-0xDC00)+0x10000;}
if(0xDC00<=code&&code<=0xDFF){return false;}
return code;}
function joinChars(tArr){var tmp=[];for(var i=0,l=tArr.length;i<l;i++){tmp.push(tArr[i]);if(i<l-1){var c1=fixedCharCodeAt(tArr[i],tArr[i].length-1);var c2=fixedCharCodeAt(tArr[i+1],0);if(!(isJaCharCode(c1)||isJaCharCode(c2))){tmp.push(' ');}}}
return tmp.join('');}
function isJaCharCode(c){return((0x3000<=c&&c<=0x305F)||(0x3040<=c&&c<=0xFF9F));}
...
return _orgText.apply(this,arguments);};function TinySegmenter(){var patterns={"[\u4e00\u4e8c\u4e09\u56db\u4e94\u516d\u4e03\u516b\u4e5d\u5341\u767e\u5343\u4e07\u5104\u5146]":"M","[\u4e00-\u9fa0\u3005\u3006\u30f5\u30f6]":"H","[\u3041-\u3093]":"I","[\u30a1-\u30f4\u30fc\uff71-\uff9d\uff9e\uff70]":"K","[a-zA-Z0-9\uff10-\uff19\uff41-\uff5a\uff21-\uff3a\!\"\#\$\%\&\'\(\)\*\+\,\-\.\/\:\;\<\=\>\?\@\[\\\]\^\_\`\{\|\}\~]":"A"}
...
var unicode_spaces=['\u0020','\u00A0','\u1680','\u180E','\u2000','\u2001','\u2002','\u2003','\u2004','\u2005','\u2006','\u2007','\u2008','\u2009','\u200A','\u200B','\u202F','\u205F','\u3000','\uFFFF'];var result=[];var seg=["B3","B2","B1"];var ctype=["O","O","O"];var o=input.split("");for(var i=0;i<o.length;++i){seg.push(o[i]);ctype.push(this.ctype_(o[i]));}
...
result.push(word);result=result.filter(function(w){return unicode_spaces.indexOf(w)===-1});return result;}})(jQuery);